...
class DataSliceTestSuite extends ApplicationManagerTestSuite {
  ...
  FileUtils.deleteDirectory(new File("tmp/dataslice"))
  FileUtils.deleteDirectory(new File("tmp/datasimulation"))
  test("test dataslice") {
  ...
  test("data validation") {
    ...
    val growthdata = sqlContext.read.parquet("tmp/dataslice")
    ...
    val datasimulationSource = sc.textFile("tmp/datasimulation/*/*")
