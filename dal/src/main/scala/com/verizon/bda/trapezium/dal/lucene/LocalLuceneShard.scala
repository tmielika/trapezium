...
//  val qp = new QueryParser("keyword", analyzer)
  ...
  override def searchDocs(queryStr: String,
  ...
  /**
    ...
    * @return
    */
   def parseQuery(queryStr: String): Query = synchronized {
     val pqp = new QueryParser("keyword", analyzer)
     pqp.parse(queryStr)
  }
  ...
  */
  override def search(queryStr: String, columns: Seq[String], sample: Double = 1.0): Iterator[Row] = {
  ...
  override def filter(docs: BitSet,
                      column: String,
                      min: Long,
                      max: Long): BitSet = {
    var i = docs.nextSetBit(0)
  ...
  override def aggregate(queryStr: String,
                         measure: String,
  ...
  override def group(queryStr: String,
                     dimension: String,
                     dimOffset: Int,
                     measure: String,
                     minTime: Long = 0L,
  ...
  override def timeseries(queryStr: String,
                          minTime: Long,
                          maxTime: Long,
                          rollup: Long,
                          measure: String,
